# E1-US4 — HTTPS via reverse proxy
...
En tant que **développeur**, je veux **HTTPS via reverse proxy** afin que **l'app mobile parle à l'API en TLS**.
...
- [x] Caddy ou Nginx + Let's Encrypt configuré
- [x] Redirection HTTP → HTTPS
- [x] Certificat valide pour le domaine ou sous-domaine
- [x] `GET /health` retourne 200 en HTTPS
