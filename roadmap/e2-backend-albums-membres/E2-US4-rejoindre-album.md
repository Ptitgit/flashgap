# E2-US4 — Rejoindre un album (POST /albums/:code/join)
...
En tant qu'**invité**, je veux **rejoindre un album avec un pseudo** afin de **participer aux uploads**.
...
- [ ] `POST /albums/:code/join` body `{ pseudo }` validé par **Zod**
- [ ] Réponse : `{ memberId, albumId, pseudo, token, expiresAt? }` — **token signé** pour `Authorization: Bearer` (cf. [conventions API](../../docs/conventions-api.md))
- [ ] Test d'intégration (Testcontainers) : join OK → token accepté sur route protégée
- [ ] 404 si code invalide
- [ ] Pseudo unique par album (409 ou suffixe auto — choix documenté)
