# E2-US3 — Créer un album (POST /albums)
...
En tant qu'**organisateur**, je veux **créer un album** afin d'**obtenir un code à partager**.
...
- [ ] `POST /albums` body optionnel `{ revealAt?: ISO8601 }`
- [ ] Réponse : `{ albumId, code, revealAt?, organizerSecret }`
- [ ] Code 6 caractères alphanumériques, collisions gérées (retry génération)
- [ ] `reveal_at` optionnel à la création
