# E6-US3 — Retry automatique (backoff + NetInfo)
...
En tant qu'**invité**, je veux **un retry automatique en cas de coupure réseau** afin de **ne pas perdre mes photos**.
...
Tests de la queue upload, backoff et transitions d'état (`pending` → `done` / `failed`) avant branchement UI.
...
- [ ] File d'attente en mémoire ; persistance minimale des pending (AsyncStorage recommandé)
- [ ] Chaque item porte un **`clientUploadId`** (UUID v4) stable entre retries — header `X-Client-Upload-Id` (BP12)
- [ ] Header `Authorization: Bearer` sur chaque tentative d'upload
- [ ] Backoff exponentiel : 1s, 2s, 4s… plafond ~5 min
- [ ] Reprise automatique au retour réseau (NetInfo)
