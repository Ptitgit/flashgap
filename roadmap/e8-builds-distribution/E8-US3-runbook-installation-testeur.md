# E8-US3 — Runbook installation testeur

| Champ | Valeur |
| --- | --- |
| **Epic** | [E8 — Builds & distribution V0 (Android + iOS)](./README.md) |
| **Statut** | `todo` |
| **Dépendances** | E5 |

## User story

En tant que **testeur**, je veux **une checklist d'installation** afin de **rejoindre l'album sans aide du dev**.
...
Ajouter/mettre à jour tests de non-régression config (app.config, eas.json) ; valider que les scripts de build échouent si config invalide.
...
- [ ] Fichier `docs/runbook-test-v0.md`
- [ ] Étapes : installer app → code album → pseudo → capturer → reveal → galerie
- [ ] Dépannage basique (réseau, permissions caméra)
...
- [ ] **Linter** : `npm run lint` dans chaque package touché (`services/api`, `apps/mobile`) — inclut ESLint **et** `no-explicit-any` — **0 erreur, 0 warning** sur les fichiers modifiés
- [ ] **Typage** : aucun `any` (eslint `@typescript-eslint/no-explicit-any` + revue) ; `npm run typecheck` dans chaque package TS touché — **0 erreur** ; types explicites sur exports publics et contrats API ([conventions TS](../../docs/conventions-typescript.md))
- [ ] **Non-régression** : `npm test` dans chaque package touché ; build dry-run documenté si pas de tests build — **100 % des tests passent** (aucun test existant en échec)
