# E4-US1 — Initialiser projet Expo (dev build)

| Champ | Valeur |
| --- | --- |
| **Epic** | [E4 — Mobile — projet Expo, join album & session](./README.md) |
| **Statut** | `todo` |
| **Dépendances** | E3, E2-US1 (`@flashgap/shared` existe) |

## User story

En tant que **développeur**, je veux **initialiser le projet Expo (TypeScript)** afin de **supporter la caméra HD en dev build**.
...
Tests composant / hook (join, session) avec API mockée — rouge puis implémentation écran + client HTTP.
...
- [ ] `expo prebuild` fonctionnel
- [ ] `tsconfig.json` étend [`tsconfig.base.json`](../../tsconfig.base.json) avec `strict: true`
- [ ] ESLint configuré : `@typescript-eslint/no-explicit-any` en `error` ; scripts `lint` et `typecheck` dans `package.json`
- [ ] Dépendance `@flashgap/shared` (`workspace:*`) ; Metro/Expo résout le package workspace ([conventions shared](../../docs/conventions-shared.md))
- [ ] Pas de dépendance à Expo Go pour les tests caméra
- [ ] Permissions : iOS `NSCameraUsageDescription`, Android `CAMERA`
- [ ] Pas de permission `MediaLibrary` en écriture / pas de save galerie
...
- Stack : Expo dev build + TypeScript + expo-camera (cadrage Q4).
