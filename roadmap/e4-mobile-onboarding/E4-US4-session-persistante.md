# E4-US4 — Session persistante (album / membre)
...
En tant qu'**invité**, je veux **rester connecté à mon album** afin de **ne pas ressaisir le code**.
...
- [ ] Persistance `memberId`, `albumId`, `pseudo`, **`token`** (SecureStore — pas AsyncStorage pour le token)
- [ ] Client HTTP injecte `Authorization: Bearer <token>` sur routes protégées
- [ ] Relance app → écran caméra si session valide
- [ ] Bouton « Quitter l'album » efface la session
